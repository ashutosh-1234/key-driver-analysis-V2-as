...
def force_refresh_regression_data() -> None:
    """Reinitialize regression data and variable selections, resetting to numeric dtypes."""
    ...
    prev_factor_names = st.session_state.get('_prev_factor_names', [])
    prev_raw_features = st.session_state.get('_prev_raw_features', [])
    factor_structure_changed = (
        (current_factor_names != prev_factor_names) or
        (current_raw_features != prev_raw_features)
    )
...
def variable_selection_interface() -> None:
    ...
    st.write(f"**Selected:** {len(st.session_state.sel_factored)} factored + {len(st.session_state.sel_raw)} raw = {nsel} total")
...
def build_safe_X() -> pd.DataFrame:
    try:
        ...
        # Factored (force numeric)
        if valid_factored:
            X_factored = st.session_state.X_factors[valid_factored].copy()
            X_factored = X_factored.apply(pd.to_numeric, errors='coerce')
...
def download_regression_data():
    X = build_safe_X()
    y = st.session_state.y_target.copy().reset_index(drop=True)
    st.write("**First rows of regression design matrix:**")
    st.dataframe(X.head(10))
    st.write("**Dtypes:**", X.dtypes)
    if not all(np.issubdtype(dtype, np.number) for dtype in X.dtypes):
        st.error("⚠️ Some columns are still non-numeric after cleaning!")
    X_clean, y_clean = get_aligned_X_y()
    st.write(f"**Rows after final align and NA drop:** {X_clean.shape[0]}")
    if not X_clean.empty and not y_clean.empty:
        ...
        st.download_button(
            ...
            file_name="regression_data_cleaned.csv",
            mime="text/csv"
        )
        with st.expander("👁️ Preview Clean Data (all numeric)"):
            st.dataframe(download_df.head(10))
    else:
        st.error("❌ No data available for download after cleaning/aligning.")
    ...
    try:
        ...
        for col in X.columns:
            if not np.issubdtype(X[col].dtype, np.number):
                st.error(f"❌ Column '{col}' is not numeric. Please use the diagnosis/download tool.")
                return
        ...
        st.info("Data may contain non-numeric or invalid values—please use the download tool to check your data.")
