...
def show_page() -> None:
    ...
    # 6 Download regression data for diagnosis
...
def build_safe_X() -> pd.DataFrame:
    """Build X matrix safely, ensuring ALL values are numeric."""
    try:
        ...
        if X_parts:
            ...
            # CRITICAL FIX: Force ALL values to be numeric by multiplying by 1
            # This converts any string numbers to actual numbers, and non-convertible to NaN
            X = X.apply(pd.to_numeric, errors='coerce') * 1
...
def download_regression_data() -> None:
    """Download regression data with complete diagnostics."""
    try:
        X = build_safe_X()
        y = st.session_state.y_target.copy().reset_index(drop=True)
        ...
        if all_numeric:
            ...
            st.error("❌ Some columns are still non-numeric!")
            non_numeric = X.dtypes[~X.dtypes.apply(lambda dt: np.issubdtype(dt, np.number))]
            st.write("Non-numeric columns:", non_numeric)
        
        # Get final aligned data
        ...
        if not X_clean.empty and not y_clean.empty:
            ...
            st.download_button(
                ...
                data=csv_data,
                file_name=f"regression_data_numeric_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.csv",
                mime="text/csv",
                help="Download cleaned numeric regression dataset"
            )
            
            # Show preview
            with st.expander("👁️ Preview Clean Data"):
                st.write("**First 10 rows:**")
                st.dataframe(download_df.head(10))
                st.write("**Final data types:**")
                st.write(download_df.dtypes)
        else:
            st.error("❌ No data available for download after cleaning")
...
def train_and_evaluate_model() -> None:
    ...
    try:
        ...
        # Verify all columns are numeric before training
        for col in X.columns:
            if not np.issubdtype(X[col].dtype, np.number):
                st.error(f"❌ Column '{col}' is not numeric. Data cleaning failed.")
                return
